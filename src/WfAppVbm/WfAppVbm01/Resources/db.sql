...
IF NOT EXISTS (
    SELECT 1
    FROM INFORMATION_SCHEMA.TABLES
    WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = 'port_info'
)
BEGIN
...
IF NOT EXISTS (
    SELECT 1
    FROM INFORMATION_SCHEMA.TABLES
    WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = 'acceleration_readings'
)
BEGIN
...
CREATE OR ALTER PROCEDURE create_measurement_info
...
CREATE OR ALTER PROCEDURE get_measurement_info
AS
BEGIN
...
CREATE OR ALTER PROCEDURE update_measurement_info
...
CREATE OR ALTER PROCEDURE delete_measurement_info
    @p_measurement_id INT
AS
BEGIN
...
CREATE OR ALTER PROCEDURE create_port_info
    @p_measurement_id INT,
...
CREATE OR ALTER PROCEDURE get_port_info
AS
BEGIN
...
CREATE OR ALTER PROCEDURE update_port_info
    @p_port_id INT,
    @p_serial_port NVARCHAR(50),
    @p_port_level INT
AS
...
CREATE OR ALTER PROCEDURE delete_port_info
    @p_port_id INT
AS
BEGIN
...
CREATE OR ALTER PROCEDURE create_acceleration_reading
    @p_measurement_id INT,
    @p_port_id INT,
...
CREATE OR ALTER PROCEDURE get_acceleration_readings
AS
BEGIN
...
CREATE OR ALTER PROCEDURE update_acceleration_reading
    @p_reading_id INT,
    @p_measurement_id INT,
    @p_port_id INT,
    @p_x_acceleration FLOAT,
...
CREATE OR ALTER PROCEDURE delete_acceleration_reading
    @p_reading_id INT
AS
BEGIN
